...
## 简介
本设计文档面向 Songloft 音频播放系统，聚焦于前端 Flutter 端的音频服务实现与播放器 UI 组件设计。系统采用 audio_service 与 just_audio 的组合方案：前者负责后台播放、通知栏控制与系统媒体控制，后者负责底层音频解码与播放控制。文档覆盖以下主题：
...
```mermaid
graph TB
A["main.dart<br/>应用入口与初始化"] --> B["core/audio/audio_service.dart<br/>SongloftAudioHandler"]
...
## 核心组件
- 音频服务处理器：SongloftAudioHandler，封装 just_audio 的 AudioPlayer，桥接 audio_service 的 PlaybackState，提供播放、暂停、跳转、下一首、上一首等控制接口，并负责通知栏元数据更新。
...
- 音频服务层：SongloftAudioHandler 通过 just_audio 播放音频，通过 audio_service 暴露系统级控制与通知栏。
...
graph TB
subgraph "音频服务层"
AS["SongloftAudioHandler<br/>just_audio + audio_service"]
end
subgraph "状态管理层"
...
### 音频服务与播放器核心（SongloftAudioHandler）
- 设计要点
...
- 控制与元数据
  - SongloftAudioHandler 将 just_audio 的事件映射为 audio_service 的 PlaybackState，系统通知栏与锁屏控制按钮可直接调用 play/pause/seek/skip 等方法。
...
## 依赖关系分析
- 组件耦合
  - PlayerNotifier 依赖 SongloftAudioHandler 的事件流与控制接口，耦合度低，职责清晰。
...
## 结论
Songloft 音频播放系统通过 audio_service 与 just_audio 的协同，实现了稳定的后台播放、系统级通知栏控制与丰富的播放器 UI。播放器状态管理采用 Riverpod Notifier，职责清晰、易于扩展。播放队列与播放模式逻辑完善，支持顺序、单曲、单曲循环与随机播放。通过事件流与管道映射，系统在性能与可靠性方面表现良好。后续可在格式支持、网络稳定性与 UI 体验方面持续优化。
