...
## 简介
本设计文档围绕 Songloft Flutter 的状态管理展开，重点说明基于 Riverpod 的 Provider 架构如何组织与驱动音频播放、认证与全局设置三大领域的状态。文档将详细解释 Provider 的定义、依赖注入与状态订阅机制，并给出音频状态（播放状态、播放列表、音量与进度）、认证状态（登录状态、用户信息与权限控制）以及全局状态（主题、语言与界面配置）的管理方式。最后提供最佳实践与常见问题排查建议。
...
subgraph "音频层"
AS["core/audio/audio_service.dart<br/>SongloftAudioHandler"]
end
subgraph "播放器特性"
...
- 依赖注入
  - 在应用入口通过 ProviderScope 将 SongloftAudioHandler 注入为 audioHandlerProvider，供播放器与其它模块使用
- 状态订阅机制
...
- 音频状态：由 SongloftAudioHandler 提供底层 just_audio 流（播放位置、时长、状态、缓冲），PlayerNotifier 订阅并映射到 PlayerState
...
participant Audio as "SongloftAudioHandler"
participant Store as "SecureStorageService"
App->>Scope : 注入 audioHandlerProvider
Scope-->>Player : 提供 SongloftAudioHandler 实例
...
- 与音频服务的桥接
  - 通过 SongloftAudioHandler 的 positionStream/durationStream/playerStateStream 等流同步 UI
...
PlayerNotifier --> SongloftAudioHandler : "订阅流/调用方法"
```
...
- 播放器依赖
  ...
  - 订阅 SongloftAudioHandler 的多个 Stream，映射到 PlayerState
- 认证依赖
...
## 故障排查指南
- 播放器无法更新进度/状态
  - 检查 SongloftAudioHandler 是否正确初始化与 ensureInitialized
...
## 结论
Songloft Flutter 的状态管理以 Riverpod 为核心，通过 Provider 的清晰分层实现了音频、认证与全局设置的解耦。借助 Notifier 的可变状态与 Provider 的依赖注入，系统在保证状态一致性的同时具备良好的扩展性与可维护性。配合资源释放与后台任务防竞态策略，整体性能与稳定性得到保障。
