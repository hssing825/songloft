...
## 引言
本设计文档面向 Songloft 的多平台适配与交付，覆盖 Android、iOS、Web、Windows、macOS、Linux 六大平台。文档从构建配置、打包与发布策略、平台特性集成（通知栏、音频服务、文件系统、硬件控制）、跨平台兼容性处理（条件编译与平台特定代码组织）等方面进行系统化梳理，并提供可视化图示与实操建议，帮助开发者高效落地与维护。
...
Xcode-->>Dev : 生成 .app
Dev->>IPA : 打包 Payload -> songloft-ios-nosign.ipa
```
...
## 结论
本设计文档基于现有配置与脚本，系统化梳理了 Songloft 在 Android、iOS、Web、Linux、macOS、Windows 六大平台的适配要点。通过统一的构建脚本与平台特定配置，结合签名、打包与分发最佳实践，可稳定支撑多端交付。后续可在 CI 中完善环境变量注入与产物归档，进一步提升自动化与一致性。
