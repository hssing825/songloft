...
## 简介
本指南面向 Songloft 项目的构建与部署，覆盖以下主题：
...
- 命名约定变更
  - 旧命名：songloft-plugin-*
  - 新命名：songloft-jsplugin-*
  - 例如：lxmusic → songloft-jsplugin-lxmusic，xiaomi → songloft-jsplugin-xiaomi
- 插件管理
...
- docker-entrypoint.sh
  ...
  - 启动时对比镜像内二进制与 /app/data/songloft 的版本，若镜像版本更新则热替换
  - **增强的版本比较算法**：支持处理 dev/unknown 特殊版本、语义化版本号比较、去除后缀字符的数字提取
  - 支持 -version 查询版本，便于版本比较
- 运行方式
  - docker build -t songloft:latest .
  - docker run -p 58091:58091 -v /your/music:/app/music -v /your/data:/app/data songloft:latest

```mermaid
flowchart TD
Start(["容器启动"]) --> CheckTarget["检查 /app/data/songloft 是否存在"]
CheckTarget --> |不存在| CopySource["复制 /app/songloft 到 /app/data/songloft"]
...
Backup --> Replace["复制镜像版本到 /app/data/songloft"]
Newer --> |否| Keep["保持现有版本"]
CopySource --> Exec["执行 /app/data/songloft 传入参数"]
Replace --> Exec
Keep --> Exec
NoUpgrade --> Exec
Upgrade --> Exec
...
- 自动发布（release.yml）
  ...
  - 上传到 songloft-org/songloft 的 GitHub Release
- **新增** SQLC 集成
...
- Docker 启动异常
  ...
  - **热升级失败**：检查 /app/data/songloft 权限与改进的版本比较逻辑
- CI/CD 失败
...
## 结论
本指南提供了 Songloft 从本地构建到 CI/CD 自动发布的完整路径，涵盖多平台二进制、Docker 镜像、前端嵌入与热升级等关键能力。**当前版本已升级到 1.3.17**，建议在生产环境中优先使用 FULL_BUILD 的完整版镜像以简化部署，并结合版本发布脚本与 GitHub Actions 实现稳定可靠的自动化发布。
...
- 常用命令速查
  ...
  - 跨平台：make build-cross GOOS=linux GOARCH=amd64 OUTPUT=build/songloft-linux-amd64
...
- **最新版本 1.3.17**（2026-04-10）
  ...
  - **命名规范**：插件命名从 songloft-plugin-* 迁移到 songloft-jsplugin-*，统一标识 JS 插件类型
...
- **命名约定标准化**
  - songloft-plugin-* → songloft-jsplugin-*
  - 区分 Go 插件和 JS 插件类型
- **开发流程优化**
  - 独立的 JS 插件开发环境
