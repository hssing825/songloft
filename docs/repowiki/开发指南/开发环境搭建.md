...
## 简介
本指南面向希望在本地搭建 Songloft 项目的开发者，涵盖系统要求、依赖安装、项目配置、开发工具推荐与配置，以及常见问题排查与不同操作系统下的特殊注意事项。项目采用 Go 作为后端、Flutter 作为跨平台客户端、Vue 3 + Vite 作为 Web 管理端，并通过 Makefile 统一管理构建与运行流程。
...
## 结论
通过本指南，您可以在不同操作系统上快速搭建 Songloft 的开发环境。建议优先使用 Makefile 提供的一键命令完成依赖安装、构建与运行；在开发模式下结合 Swagger 文档与 Vite 代理提升调试效率；在生产环境使用 Docker 容器化部署并配置镜像加速以提高构建稳定性。
