...
## 简介
本文件为 Songloft 音乐扫描管理 API 的详细接口文档，覆盖以下能力：
...
## 结论
Songloft 的扫描管理 API 以异步启动、实时进度查询为核心，结合预过滤、并发提取与批量导入等优化策略，能够高效处理大规模音乐库。通过清晰的状态模型与错误处理机制，配合前端轮询与取消控制，为用户提供了稳定可靠的扫描体验。建议在生产环境中合理配置扫描路径与并发参数，并在大库扫描时启用重新导入模式以确保元数据一致性。