...
## 简介
本文件为 Songloft 令牌管理接口的详细 API 文档，聚焦以下两个接口：
...
## 结论
Songloft 的令牌管理接口通过清晰的分层设计与完善的权限控制，提供了安全可靠的令牌生命周期管理能力。列表查询支持类型过滤、排序与分页，撤销接口支持撤销原因记录与撤销者识别。配合数据库索引与缓存策略，整体具备良好的性能与可维护性。建议在生产环境中结合前端分页参数与后端索引策略，持续优化查询性能与用户体验。
