...
- [plugins/songloft-plugin-xiaomi/schedule/scheduler.go](file://plugins/songloft-plugin-xiaomi/schedule/scheduler.go)
...
## 简介
本指南面向 Songloft 的运维与开发团队，提供一套完整的监控与日志配置方案。内容覆盖：
- 日志配置：日志级别、格式、轮转与采集
...
## 结论
本指南提供了 Songloft 的日志与监控落地路径：以结构化日志为基础，结合健康检查、数据库与插件层面的可观测性，配合容器化与外部监控栈，形成完整的监控闭环。JavaScript 运行时日志级别优化进一步提升了系统的可观测性和性能表现。建议在生产环境补充自定义指标与告警规则，持续优化性能与稳定性。
