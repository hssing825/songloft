...
Songloft 是一个基于 Go 和 Chi 框架的轻量级音乐服务器，支持本地音乐文件管理、元数据提取和歌单管理。本文档提供了 Songloft 的完整 Docker 部署指南，涵盖多阶段构建策略、Go 编译优化、缓存机制和体积优化等关键主题。

Songloft 提供了现代化的 Docker 容器化解决方案，包括：
- 多阶段 Docker 构建
- Go 编译优化和缓存机制
- 生产环境和开发环境的不同构建选项
- 完整的 CI/CD 集成
...
Songloft 项目采用模块化的组织结构，主要包含以下关键目录：
...
Songloft 采用了精心设计的多阶段 Docker 构建策略，实现了高效的构建流程和最小化的镜像体积。
...
Songloft 实现了多层次的缓存优化策略，显著提升了构建效率：
...
Songloft 的 Docker 架构采用分层设计，实现了构建时和运行时的最佳分离：
...
Songloft 提供了两种构建模式，通过 `FULL_BUILD` 参数控制：
...
**新增功能** Songloft 现在支持 VERSION 构建参数，用于传递和注入应用版本信息：
...
```bash
docker inspect --format "{{json .State.Health }}" songloft
```
...
**新增功能** Songloft 现在集成了完整的自动化发布流程，包括版本管理和变更日志生成：
...
# 重新构建（忽略缓存）
docker build --no-cache -t songloft .
```
...
```bash
# 设置 GOPROXY
docker build --build-arg GOPROXY=https://proxy.golang.org -t songloft .
...
```bash
# 检查 VERSION 参数传递
docker build --build-arg VERSION=v1.3.13 -t songloft:v1.3.13 .

# 验证版本注入
docker run --rm songloft:v1.3.13 /app/songloft version
```
...
# 查看最近日志
docker logs --tail 100 songloft

# 查看错误日志
docker logs --tail 100 --since "1h" songloft
```
...
Songloft 提供了智能的热升级功能，允许在不停机的情况下更新应用程序：
...
Songloft 的 Docker 部署方案展现了现代容器化应用的最佳实践。通过精心设计的多阶段构建、智能的缓存策略和全面的安全加固，实现了高效、可靠且易于维护的部署体验。
...
```bash
# 基础构建
docker build -t songloft .

# 完整版本构建
docker build --build-arg FULL_BUILD=true -t songloft-full .

# 带 Git 信息的构建
docker build \
  --build-arg GIT_COMMIT=$(git rev-parse --short HEAD) \
  --build-arg BUILD_TIME=$(date -u '+%Y-%m-%d_%H:%M:%S') \
  -t songloft-with-info .
...
| `DB_PATH` | 字符串 | `data/songloft.db` | 数据库路径 |
