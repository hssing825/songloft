...
## 简介
本文件系统性梳理 Songloft 的认证安全机制，重点覆盖：
...
## 结论
Songloft 的认证安全机制采用 JWT 双 Token 模式，结合内存缓存与 SQLite 撤销/过期检查，实现了访问令牌的短期高可用与刷新令牌的长期可控。前端通过统一拦截器与自动刷新提升用户体验，后端中间件与处理器确保请求链路的严格认证。建议进一步完善 CSRF 防护与安全头配置，以增强整体安全性。
